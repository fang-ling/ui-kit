...
/// attributed strings to customize the appearance of substrings within the
/// text.
///
...
@MainActor public class Label: View {
  ...
  public override func display(_ layer: Layer) {
    super.display(layer)
    ...
    // UIKit does not draw text outside the frame;
    ...
    if let text {
      JavaScriptBridge.updateElementTextContent(
        elementID: layer.contents,
        text: text
      )
    }
